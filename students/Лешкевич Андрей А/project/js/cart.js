...
class CCart {
    ...
    updateItem(_id, _t, _val) {
        ...
        this.items.find(obj => { return (obj.id === _id && obj.type === _t) }).quantity = _val;
...
class CCartSmallHTML {
    ...
    itemGen(_obj) {
        return `<li ${this.attrs.item}="${_obj.id}" ${this.attrs.type}="${_obj.type}" class="btn-cart__ddc-item">
    ...
    updateItem(_id, _t) {
        ...
        let price = this.cart.catalog.find(obj => { return obj.id === _id }).type[_t].price;
